...
/// @param [in]     heatGeneratedOn            (W)     Thermal heat generated by IPS
...
GunnsElectIpsConfigData::GunnsElectIpsConfigData(const std::string& name,
    ...
    mheatGeneratedOn(heatGeneratedOn),
...
GunnsElectIpsConfigData::GunnsElectIpsConfigData(const GunnsElectIpsConfigData& that)
    ...
    mheatGeneratedOn(that.mheatGeneratedOn),
...
GunnsElectIps::GunnsElectIps()
    :
    GunnsBasicLink(1),
    mFailPowerInputMalfunction(0),
    mFailAllPowerInputsMalfucntion(false),
    mMalfBiasPowerConsumedOn(false),
    mMalfBiasPowerConsumedOnValue(0.0),
    ...
    mBackUpPowerSource(GunnsElectIps::Invalid_Source),
...
void GunnsElectIps::cleanup() {
    ...
    TS_DELETE_ARRAY(mFailPowerInputMalfunction);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void GunnsElectIps::allocateArrays() {
    cleanup();
    TS_NEW_PRIM_ARRAY_EXT(mFailPowerInputMalfunction, mNumberOfPowerSources, bool,   std::string(mName) + ".mFailPowerInputMalfunction");
    ...
    for (int i = 0; i < mNumberOfPowerSources; i++) {
        mFailPowerInputMalfunction[i] = false;
                               ...
                               std::vector<int>*             portsVector)
{
    ...
    if (GunnsElectIps::Minimum_Number_Of_Power_Sources > mNumberOfPowerSources) {
    ...
    mheatGeneratedOn = configData.mheatGeneratedOn;
...
int GunnsElectIps::getVoltagePrimarySourceGreaterThanBackupThreshold() {
    int sourceToUse = GunnsElectIps::Invalid_Source;
    double sourceVoltage = 0.0;
    for (int source = 0; source < mBackUpPowerSource; source++) {
        if (!mFailPowerInputMalfunction[source] &&
...
int GunnsElectIps::getVoltagePrimarySourceGreaterUnderVoltageLimit() {
    ...
    if (sourceToUse != GunnsElectIps::Invalid_Source) {
        if (!mFailPowerInputMalfunction[mBackUpPowerSource] && !mFailAllPowerInputsMalfucntion) {
            if ((mCommandOnUsed && mCommandOn) || (!mCommandOnUsed)) {
...
int GunnsElectIps::getVoltageSourceToUseWithBackup() {
    ...
    if (firstVoltage != GunnsElectIps::Invalid_Source) {
        sourceToUse = firstVoltage;
    } else {
        int secondVoltage = getVoltagePrimarySourceGreaterUnderVoltageLimit();
        if (secondVoltage != GunnsElectIps::Invalid_Source) {
...
int GunnsElectIps::getVoltageSourceToUseWithoutBackup() {
    int sourceToUse = 0;
    double sourceVoltage = 0.0;
    if (GunnsElectIps::Invalid_Source == mActivePowerSource) {
        sourceToUse = GunnsElectIps::Invalid_Source;
    } else {
        sourceToUse = mActivePowerSource;
        if ((mCommandOnUsed && mCommandOn) || (!mCommandOnUsed)) {
            if (!mFailPowerInputMalfunction[mActivePowerSource] && !mFailAllPowerInputsMalfucntion) {
    ...
    if ((mCommandOnUsed && mCommandOn) || (!mCommandOnUsed)) {
        for (int source = 0; source < mNumberOfPowerSources; source++) {
            if (!mFailPowerInputMalfunction[source] && !mFailAllPowerInputsMalfucntion) {
...
void GunnsElectIps::calculateFlow(const int sourceToUse) {
    ...
    if (sourceToUse != GunnsElectIps::Invalid_Source) {
        ...
        mCurrent = 0.0;
        mActivePowerSource = GunnsElectIps::Invalid_Source;
        mControlInputPowerInUse = false;
...
void GunnsElectIps::computeFlows(const double TimeStep) {
    ...
    if (mMalfBiasPowerConsumedOn){
        mPowerConsumedOn = mDefaultPowerConsumedOn + mMalfBiasPowerConsumedOnValue;
    } else {
    ...
    mBackupVoltageInRange = (getVoltageBackupWithinRange() != GunnsElectIps::Invalid_Source) && mBackUpPowerImplemented;
...
GunnsBasicLink::SolutionResult GunnsElectIps::confirmSolutionAcceptable(const int convergedStep,
    ...
    if (convergedStep > 0) {
        ...
        if (mVoltageSwitches < mNumberOfVoltageSwitchesInASolution) {
            ...
            if (GunnsElectIps::Invalid_Source == mLastActivePowerSource &&
                    sourceToUse !=  GunnsElectIps::Invalid_Source){
                mLastActivePowerSource = mActivePowerSource;
            }
...
void GunnsElectIps::setMalfPowerInput(const int input, const bool value)
{
    if (0 <= input and mNumberOfPowerSources > input) {
        mFailPowerInputMalfunction[input] = value;
    } else {
...
{
    mFailAllPowerInputsMalfucntion = flag;
}
